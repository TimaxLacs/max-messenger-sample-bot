...
Проект поддерживает 3 режима (задаются через `BOT_MODE` в `.env`):
1. **Режим 1 (Минимальный, `BOT_MODE=1`)**: Отправил фото — получил фото (через оркестратор).
2. **Режим 2 (Полный, `BOT_MODE=2`)**: Меню с кнопками (Фильтры, Фоны, Исторические личности, Надписи). Бот просит фото после выбора и передаёт в разные оркестраторы (например, `ORCHESTRATOR_URL_FILTERS`).
3. **Режим 3 (Памятная открытка, `BOT_MODE=3`)**: Праздничная подача для 9 мая.
...
|-----------|-------------|
| `src/bot1-minimal.mjs` | Логика режима 1 (минимальный бот) |
| `src/bot2-full.mjs` | Логика режима 2 (полный бот с меню) |
| `src/bot3-memorial.mjs` | Логика режима 3 (бот 9 мая) |
| `src/shared.mjs` | Общие функции бота |
